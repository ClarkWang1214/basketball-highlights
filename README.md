...
正好之前在折腾小米 Mimo v2.5 模型做图片理解（[mimo-vision-mcp](https://github.com/ClarkWang1214/mcp-mimo-vision)），发现它支持视频输入。核心问题就一个——Mimo 能不能从篮球视频里认出"进球了"并告诉你第几秒？
...
> Mimo v2.5 从视频中识别到的单次进球全过程——组织进攻 → 投篮出手（最高点）→ 球接近篮筐。检测到进球时间戳后，自动截取 ±5s/+3s 片段，拼接为集锦。
